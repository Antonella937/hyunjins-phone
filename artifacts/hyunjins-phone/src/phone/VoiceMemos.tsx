...
export function VoiceMemos({ store, setStore, detail, setDetail, editMode }: {
  ...
  useEffect(() => {
    ...
    setPlaying(false);
    setCurrentTime(0);
    setDuration(memo?.durationSeconds || 0);
  ...
  if (memo) return <div>
    ...
    <div className="flex items-center gap-3"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#8b91ba] text-[#242038]"><Mic2 size={24} /></span><div><p className="text-xs text-white/45">{memo.date} · {memo.duration}</p><h1 className="mt-1 font-serif text-3xl">{memo.title}</h1></div></div>
    ...
    {memo.audioId && <audio key={memo.audioId} ref={audioRef} src={audioUrl(memo.audioId)} preload="metadata"
      ...
      onError={() => setError('Audio could not be loaded. Please try regenerating it.')} />}
    ...
    {memo.audioId && <input aria-label="Seek audio" type="range" min={0} max={duration || memo.durationSeconds || 1} step={0.1} value={Math.min(currentTime, duration || memo.durationSeconds || 1)} onChange={e => { if (audioRef.current) audioRef.current.currentTime = Number(e.target.value); setCurrentTime(Number(e.target.value)); }} className="mt-3 w-full accent-[#d98ca7]" />}
    <div className="mt-5 flex items-center justify-between text-xs text-white/40">
      <button onClick={playPause} disabled={!memo.audioId} className="flex items-center gap-2 rounded-full bg-[#d98ca7] px-4 py-2 text-[#291d26] disabled:opacity-40" data-testid="button-play-voice">{playing ? <Pause size={14} fill="currentColor" /> : <Play size={14} fill="currentColor" />}{playing ? 'pause' : 'play'}</button>
      <span>{formatTime(currentTime)} / {memo.audioId ? formatTime(duration || memo.durationSeconds || 0) : memo.duration}</span>
      <button onClick={() => { const next = speed === 1 ? 1.25 : speed === 1.25 ? 1.5 : speed === 1.5 ? 0.75 : 1; setSpeed(next); if (audioRef.current) audioRef.current.playbackRate = next; }} className="rounded-full border border-white/15 px-2 py-1">{speed}×</button>
    </div>
