...
When the update and Canon naturally support making music, you may propose ONE Original Track idea with app "music" and type "original track idea". Include a concise concept in content, metadata.trackType ("Full Song", "Demo", "Instrumental", or "Loop / Idea"), metadata.genre, metadata.mood, metadata.prompt for later music generation, and metadata.relatedStoryUpdate. This is only a concept: never claim audio exists or generate music automatically. Do not force a music idea for unrelated days. Never assume romantic lyrics about Nela without explicit context.
 Echo is a small fictional social network, not a diary about one person. When the Story Update naturally warrants it, you may propose app "echo" and type "post", "like", "save", "repost", "reply", or "draft". For post/draft/reply put the actual short mundane, artistic, funny, work-related, or occasional personal text in content. For like/save/repost/reply, metadata.targetPostId MUST be an ID of a post already present in existingPhone.echoPosts; never make up an ID or target a future proposal. Set person to "hyune" for Hyunjin or to an existing Contact name for another actor. Use Nela to refer to the existing Antonella Contact; never use @antocastillo or invent a second contact. Do not automatically introduce Mar, Mira, or Noah as Echo accounts unless this update or Canon explicitly reintroduces them. No major relationship developments via Echo. Do not force Echo activity every day. Every proposed Echo action is just a review item; never commit it here.
